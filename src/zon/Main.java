...
public class Main {
    ...
    private static boolean LoadFromBinary(TMD3Model MD3Model, String FileName) {
        try (RandomAccessFile Source = new RandomAccessFile(FileName, "r")) {
            ...
            for (int FrameIndex = 0; FrameIndex < MD3Model.NumberOfFrames; FrameIndex++) {
                ...
                TMD3ModelFrame MD3ModelFrame = new TMD3ModelFrame(MinBound, MaxBound, LocalOrigin, Radius, Name);
                MD3Model.ModelFrames.add(MD3ModelFrame);
            }
            // Read the tags.
            ...
            for (int TagIndex = 0; TagIndex < MD3Model.NumberOfTags; TagIndex++) {
                ...
                TMD3Tag MD3FrameTag = new TMD3Tag(Name, Location, OrientationX, OrientationY, OrientationZ);
                MD3Model.Tags.add(MD3FrameTag);
            }
            // Read the surfaces.
    ...
    private static boolean LoadFromText(TMD3Model MD3Model, String FileName) {
        ...
        try (BufferedReader Source = new BufferedReader(new FileReader(FileName))) {
            ...
            while ((InputLine = Source.readLine()) != null) {
                ...
                if (InputTokens.length > 0) {
                    ...
                    if (InputTokens[0].equalsIgnoreCase("tp")) {
                        MD3Tag.Location.X = Float.parseFloat(InputTokens[1]);
                        MD3Tag.Location.Y = Float.parseFloat(InputTokens[2]);
                        MD3Tag.Location.Z = Float.parseFloat(InputTokens[3]);
                    }
                    if (InputTokens[0].equalsIgnoreCase("tr")) {
                        MD3Tag.OrientationX.X = Float.parseFloat(InputTokens[1]);
                        MD3Tag.OrientationX.Y = Float.parseFloat(InputTokens[2]);
                        MD3Tag.OrientationX.Z = Float.parseFloat(InputTokens[3]);
                        MD3Tag.OrientationY.X = Float.parseFloat(InputTokens[4]);
                        MD3Tag.OrientationY.Y = Float.parseFloat(InputTokens[5]);
                        MD3Tag.OrientationY.Z = Float.parseFloat(InputTokens[6]);
                        MD3Tag.OrientationZ.X = Float.parseFloat(InputTokens[7]);
                        MD3Tag.OrientationZ.Y = Float.parseFloat(InputTokens[8]);
                        MD3Tag.OrientationZ.Z = Float.parseFloat(InputTokens[9]);
                    }
            ...
            // Process frames.
            for (int FrameIndex = 0; FrameIndex < MD3Model.NumberOfFrames; FrameIndex++) {
                TMD3ModelFrame MD3ModelFrame = MD3Model.ModelFrames.get(FrameIndex);
                float MaxX = -655360;
                float MaxY = -655360;
                float MaxZ = -655360;
                float MinX = 655360;
    ...
    private static boolean SaveToBinary(TMD3Model MD3Model, String FileName) {
        ...
        int OffsetOfSurfaces = OffsetOfTags + MD3Model.NumberOfTags * 112;
        int SurfaceSize = 0;
        for (int SurfaceIndex = 0; SurfaceIndex < MD3Model.NumberOfSurfaces; SurfaceIndex++) {
            ...
            SurfaceSize +=
                    ...
                    MD3Surface.MD3TextVertexes.size() * MD3Model.ModelFrames.size() * 8; // Frames vertices array size.
        }
        ...
        try (DataOutputStream Writer = new DataOutputStream(new FileOutputStream(FileName))) {
            ...
            // Write the tags.
            for (int TagIndex = 0; TagIndex < MD3Model.NumberOfTags; TagIndex++) {
                TMD3Tag MD3Tag = MD3Model.Tags.get(TagIndex);
            ...
            for (int SurfaceIndex = 0; SurfaceIndex < MD3Model.NumberOfSurfaces; SurfaceIndex++) {
                ...
                int SurfaceOffsetEnd = OffsetOfVertices + MD3Surface.MD3TextVertexes.size() * MD3Model.ModelFrames.size() * 8;
                ...
                // Read the space coordinates.
                for (int VerticeIndex = 0; VerticeIndex < MD3Surface.MD3TextVertexes.size() * MD3Model.ModelFrames.size(); VerticeIndex++) {
    ...
    private static boolean SaveToText(TMD3Model MD3Model, String FileName) {
        //try (FileOutputStream FOS = new FileOutputStream(OutputName); PrintWriter Writer = new PrintWriter(FOS)) {
        try (FileWriter Writer = new FileWriter(FileName)) {
            Writer.write(MD3Model.toString());
            Writer.flush();
        } catch (IOException Ignored) {
            return false;
        }
